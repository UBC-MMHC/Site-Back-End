    ...
    private static Map<String, Object> parseDatabaseUrl(String url) {
        try {
            if (url.startsWith("jdbc:")) {
                return null; // Already JDBC, let default config handle it
            }
            String normalized = url.startsWith("postgres://") ? "postgresql://" + url.substring(11) : url;
            // Parse manually: password may contain @ or :, so split from the right
            ...
            // hostPart: host:port/database or host/database
            String host;
            int port = 5432;
